...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      ...
      <form onSubmit={submit} className="mt-6 rounded-[1.75rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 sm:p-6">
        <div className="grid gap-3 sm:grid-cols-[0.55fr_1fr]">
          <input
            ...
            maxLength={60}
            className="h-12 w-full rounded-full border border-[var(--tk-line)] bg-[var(--tk-raised)] px-4 text-sm text-[var(--tk-text)] outline-none"
          />
          <textarea
            value={text}
            ...
            rows={3}
            maxLength={1500}
            className="w-full resize-y rounded-[1.2rem] border border-[var(--tk-line)] bg-[var(--tk-raised)] px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none"
          />
        </div>
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-6 py-3 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
          >
